...
`herdr.tab` puts the board in its own herdr tab (found or created by label; boards sharing a label share the tab). `herdr.workspace` picks the workspace by label or id, and creates it by label if it is missing (default: the focused one). `parent` (a pane id or `board:<name>`) splits that pane instead. `"enabled": false` closes the pane but keeps the file.
